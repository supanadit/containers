...
configure_pgbackrest() {
    ...
    local repo1_path_override="${PGBACKREST_REPO_PATH:-}" # Optional explicit override

    # Create pgBackRest configuration
    cat > "$config_file" << EOF
# pgBackRest configuration generated by container
[global]
repo1-retention-full=${PGBACKREST_REPO_RETENTION_FULL:-2}
repo1-retention-diff=${PGBACKREST_REPO_RETENTION_DIFF:-6}
...
EOF
    ...
    if [ "${repo1_type}" = "posix" ] || [ "${repo1_type}" = "filesystem" ]; then
        ...
        [ -n "${PGBACKREST_REPO_S3_BUCKET:-}" ] && echo "repo1-s3-bucket=${PGBACKREST_REPO_S3_BUCKET}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_ENDPOINT:-}" ] && echo "repo1-s3-endpoint=${PGBACKREST_REPO_S3_ENDPOINT}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_REGION:-}" ] && echo "repo1-s3-region=${PGBACKREST_REPO_S3_REGION}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_KEY:-}" ] && echo "repo1-s3-key=${PGBACKREST_REPO_S3_KEY}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_KEY_SECRET:-}" ] && echo "repo1-s3-key-secret=${PGBACKREST_REPO_S3_KEY_SECRET}" >> "$config_file"
        ...
        [ -n "${PGBACKREST_REPO_S3_URI_STYLE:-}" ] && echo "repo1-s3-uri-style=${PGBACKREST_REPO_S3_URI_STYLE}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_PORT:-}" ] && echo "repo1-s3-port=${PGBACKREST_REPO_S3_PORT}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_STORAGE_CLASS:-}" ] && echo "repo1-s3-storage-class=${PGBACKREST_REPO_S3_STORAGE_CLASS}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_TOKEN:-}" ] && echo "repo1-s3-token=${PGBACKREST_REPO_S3_TOKEN}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_CA_FILE:-}" ] && echo "repo1-s3-ca-file=${PGBACKREST_REPO_S3_CA_FILE}" >> "$config_file"
        [ -n "${PGBACKREST_REPO_S3_CA_PATH:-}" ] && echo "repo1-s3-ca-path=${PGBACKREST_REPO_S3_CA_PATH}" >> "$config_file"
    else
