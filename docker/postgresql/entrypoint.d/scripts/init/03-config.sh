...
copy_user_configs() {
    local data_dir="${PGDATA:-/opt/containers/data}"
    local config_dir="${PGCONFIG:-/opt/containers/config}"
...
generate_secure_defaults() {
    ...
    local custom_config_dir="/opt/containers/config/custom"
    ...
    local custom_hba_dir="/opt/containers/config/hba"
...
# include_dir '/opt/containers/config/hba'
...
apply_environment_overrides() {
    ...
    apply_postgres_setting "unix_socket_directories" "${POSTGRESQL_UNIX_SOCKET_DIRECTORIES:-${PGRUN:-/opt/containers/run}}"
...
apply_native_ha_config() {
    if [[ "${HA_MODE:-}" == "native" ]]; then
        ...
        if [[ "${REPLICATION_ROLE:-}" == "primary" ]]; then
            local hba_file="${PGDATA:-/opt/containers/data}/pg_hba.conf"
...
postgresql:
    ...
    parameters:
        unix_socket_directories: '${PGRUN:-/opt/containers/run}'
        timezone: '${POSTGRESQL_TIMEZONE:-UTC}'
EOF_FOOTER
