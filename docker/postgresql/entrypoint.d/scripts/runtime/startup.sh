...
start_pgbouncer() {
  ...
  sed -i "s|\${PGBOUNCER_LISTEN_ADDR}|${PGBOUNCER_LISTEN_ADDR}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_LISTEN_PORT}|${PGBOUNCER_LISTEN_PORT}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_AUTH_TYPE}|${PGBOUNCER_AUTH_TYPE}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_ADMIN_USERS}|${PGBOUNCER_ADMIN_USERS}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_STATS_USERS}|${PGBOUNCER_STATS_USERS}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_POOL_MODE}|${PGBOUNCER_POOL_MODE}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_MAX_CLIENT_CONN}|${PGBOUNCER_MAX_CLIENT_CONN}|g" "$config_file"
  sed -i "s|\${PGBOUNCER_DEFAULT_POOL_SIZE}|${PGBOUNCER_DEFAULT_POOL_SIZE}|g" "$config_file"

  echo "" >>"$config_file"

  compgen -A variable | grep "^PGBOUNCER_CONFIG_" | while read -r var_name; do
    # Convert PGBOUNCER_CONFIG_MAX_CLIENT_CONN -> max_client_conn
    key_name=$(echo "${var_name#PGBOUNCER_CONFIG_}" | tr '[:upper:]' '[:lower:]')
    value="${!var_name}"

    # Check if the key already exists in the file (uncommented)
    if grep -q "^${key_name}[[:space:]]*=" "$config_file"; then
      # If it exists, replace the whole line with the new value
      # This ensures PGBOUNCER_CONFIG_ overrides the template default
      sed -i "s|^${key_name}[[:space:]]*=.*|${key_name} = ${value}|g" "$config_file"
    else
      # If it doesn't exist, append it safely to the end
      # Ensure a newline exists first
      echo "" >> "$config_file"
...
wait_for_postgresql_ready() {
  ...
  while [ $attempt -le $max_attempts ]; do
    # Use Unix socket connection (no -h flag) for reliable local connectivity
    if su -s /bin/bash postgres -c "pg_isready -U ${POSTGRES_USER:-postgres}" >/dev/null 2>&1; then
