...
function backupFlags(type) {
	...
	// Default-true flags: ezx env.isTruthy has no default param, so use the
	// env.js truthy helper to honor the shell's default-true semantics.
...
function primaryGate() {
	...
	return {
		...
		exec: [
			"/bin/sh",
			"-c",
			"PGPASSWORD='" + (env.get("POSTGRES_PASSWORD") || "") + "' psql -qtAX -U " +
...
function backupNode(type, defaultCron) {
	return {
		...
		scheduler: scheduler.build({
			...
			gate: env.isTruthy("PGBACKREST_AUTO_PRIMARY_ONLY", "true") ? primaryGate() : null,
		}),
	};
}
