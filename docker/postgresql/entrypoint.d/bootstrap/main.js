...
const { registerOpsRoutes } = require("./routes");
const { registerPatroniRoutes } = require("./patroni");
...
function main() {
	...
	// Register operator-facing routes (sync-reload, composite health, Patroni
	// role-change) on the health server before the chain starts supervising.
	registerOpsRoutes();
	registerPatroniRoutes();
	startDatabase();
